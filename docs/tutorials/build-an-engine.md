...
* read from a file
* queried from an SKDS (see [Ecosystem](../main-concepts/ecosystem.md) for more info)
...
4. Perform the parameter and operation [RML](https://rml.io/) Mapping(s) - defined in the Mapping's `parameterMappings` and `operationMappings` properties respectively - to obtain the unique parameters and operation to perform the Verb using an Integration's API.
5. Perform the Verb’s operation using the Integration's API.
6. Obtain the response from performing the Verb’s operation.
7. Perform the return value [RML](https://rml.io/) Mapping(s) - defined in the Mapping's `returnValueMappings` property - to obtain the return value of the Verb.
8. Ensure the validity of the return value according to the [SHACL Shape](https://www.w3.org/TR/shacl/) identified by the Verb’s `returnValue` property.
9. Return the output of the Verb.
